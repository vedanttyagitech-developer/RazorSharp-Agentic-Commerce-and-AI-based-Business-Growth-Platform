...
# durable-worker: the only process that talks to Razorpay (ADR 0003 D3). Build from the
# REPOSITORY ROOT:
#
#   docker build -f infra/docker/durable-worker.Dockerfile -t durable-worker:dev .
#
...
ARG WORKER_MODULE=durable_worker.main
...
ARG VERSION=0.1.0
LABEL org.opencontainers.image.title="durable-worker" \
...
# Contract for durable_worker.main (docs/DEPLOY.md, "Contracts the services must honour"):
