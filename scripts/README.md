...
| `run_demo.sh` | reads `.env`, targets `commerce_dev`; any database via `DEMO_DB` | Starts the API and the Action Executor together and stops both cleanly on Ctrl-C. `--api-only` and `--worker-only` are what `make api` and `make worker` call. `commerce_dev` and `commerce_test` need nothing but `DEMO_DB`; any other database runs on the three `DATABASE_URL_APP` / `_KERNEL` / `_WORKER` URLs you export, which win over `.env`. That plus a free `PORT` is a stack of your own to inject failures into — see `scripts/run_demo.sh --help`. |
...
sequence a buyer, an agent and the Action Executor perform between them. A seeder that
...
| `DATABASE_URL_TEST_WORKER` | the action-executor suites and the role-privilege suites, as `commerce_test_worker` |
