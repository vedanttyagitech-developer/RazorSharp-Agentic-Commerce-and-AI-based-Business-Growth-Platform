...
| Kernel | T: Safe Mode activation racing an in-flight admission | Shared transaction advisory gates for money actions; exclusive scope gate for mode writes; consume-time Reserve check | Implemented | `test_activation_waits_for_admission_transaction`, `test_execution_waits_for_activation_then_refuses` | Tests cover tenant and global scopes. Already consumed external operations remain reconcilable. |
