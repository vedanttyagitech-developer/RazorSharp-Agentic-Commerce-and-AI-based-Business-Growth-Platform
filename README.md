...
### Copilot
Gemini on Vertex AI. Three specialists with closed, enumerated action sets. Deterministic routing and deterministic language detection: no model decides who
answers or what language you spoke. Falls back to a deterministic runner when Vertex is
unconfigured, and says so rather than going quiet.
...
of them can reach kernel admission**, and it still cannot move money by itself. No tool can
name an amount: `ArgumentKind` has no monetary member, which is what makes
`order.propose_cancellation` and `refund.propose` honest. **ACP** — five routes,
signature-verified over a length-prefixed canonical string, with replay protection as a
database uniqueness guarantee rather than an in-memory set.

### Demonstrating failure, on purpose
A scenario surface can arm real faults — provider timeouts, stale captures, transport errors
— so the recovery paths can be shown rather than described. Faults disarm themselves in the
same update that consumes them, so a demo cannot leave one armed.
...
Current state: **mypy `--strict` clean across 278 source files**, front end **175 tests
passing** with TypeScript clean. Backend test counts come from the run artifacts rather than
a number maintained by hand here, because a hand-maintained number rots.
...
- **Skips in the default run are environmental** — they guard on PostgreSQL being reachable.
  The voice wire-contract test is *not* among them: it reads
  `apps/razorsharp-concept/lib/voice/wire.ts` and holds the server's frame contract against
  the front end that actually ships.
