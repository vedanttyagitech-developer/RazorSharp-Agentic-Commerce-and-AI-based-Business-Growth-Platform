...
``python -m action_executor`` runs until it is told to stop. ``--once`` runs a single pass
and exits, which is what a scheduled job, a smoke test and a demonstration script all
want, and what makes "did the executor do anything" answerable without tailing a log.

``SIGTERM`` and ``SIGINT`` set a flag rather than raising. Kubernetes sends ``SIGTERM``
and then waits; an executor that died where the signal found it could be halfway between
...
_LOG: Final = logging.getLogger("action_executor.main")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="action-executor",
