"""Make ``python -m action_executor`` mean what :mod:`action_executor.main` says it means.

``main.py`` has documented ``python -m action_executor`` as this process's invocation since
...
action_executor.__main__`` and the working form -- ``python -m action_executor.main`` -- was
the one nobody had written down. That is a bad way to lose an executor: this is the only
...
The module keeps no logic of its own. It defers to :func:`action_executor.main.main` so the
