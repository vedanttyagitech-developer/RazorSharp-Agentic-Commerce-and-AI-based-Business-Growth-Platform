...
from action_executor.transport import (
    PROVIDER_LABEL,
    HttpxTransport,
    provider_operation,
)
...
def json_log() -> Iterator[io.StringIO]:
    """Capture ``action_executor``'s own records the way the running process renders them.
...
class TestTheProviderCallIsMeasuredWithoutBeingChanged:
    def test_a_status_the_provider_answered_with_is_classified_not_reported_raw(
        ...
        with pytest.MonkeyPatch.context() as patch:
            patch.setattr(
                "action_executor.transport.default_registry",
    ...
    def test_a_timeout_still_raises_the_exception_the_adapter_classifies(
        ...
        with pytest.MonkeyPatch.context() as patch:
            patch.setattr("action_executor.transport.default_registry", lambda: clean_registry)
            from platform_observability import bind_scope
