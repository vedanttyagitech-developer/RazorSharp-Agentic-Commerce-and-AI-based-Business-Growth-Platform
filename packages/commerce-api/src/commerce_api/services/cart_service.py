...
A cart is buyer intent and nothing else: a set of SKUs and whole-unit quantities. It
...
stored copy on ``carts.quote`` exists only so the buyer surface can show what was last
...
**Staleness is reported, not hidden.** The cart stores the catalogue revision it was
...
**A line write locks the cart row.** ``SELECT ... FOR UPDATE`` before the read-modify-
write, so two tabs setting quantities on one cart serialize instead of one silently
...
this cart would produce, built through
...
them, so the cart stamps its own id and version 1 in their place. Everything else --
...
#: A quick-commerce cart. Bounded because every line is re-priced and re-reserved on
#: every checkout, and an unbounded cart is an unbounded admission transaction.
MAX_BASKET_LINES = 40
...
def stored_lines(cart: Cart) -> list[dict[str, Any]]:
    """The cart's lines as stored: ``[{"sku": str, "quantity": int}, ...]``, SKU order.
...
def preview_content_hash(quote: Quote, *, cart_id: uuid.UUID, policy_version: str) -> str:
    """The canonical hash this cart would produce as checkout version 1.

    See the module docstring: the cart's own id stands in for a checkout id that does
...
class ExpectedCart:
    ...
    RazorAI reads a cart and a product, shows the buyer what adding a line would mean,
    and the buyer presses confirm some seconds or minutes later. In between, another tab
    can edit the same cart and the merchant can move a price or the whole catalogue.
    ...
    ``basket_content_hash`` is the cart's *current* preview hash -- the canonical bytes
    of the checkout this cart would produce as version 1, exactly as
    :class:`~commerce_api.schemas.QuoteOut` carries it on every read. ``None`` is not a
    missing value: it is the positive claim that the cart held nothing priceable, which
    is what an empty cart's quote is.

    Binding the whole cart rather than the one line is deliberate. If another surface
    added onions between the proposal and the press, this line's price has not moved but
    the cart the buyer was shown has, and a line-scoped check would let a card execute
    beside a total it can no longer stand behind.

    What this proves, precisely: the cart, this product's price and the catalogue are
...
def cart_binding(cart: Cart, *, registry: MerchantRegistry) -> dict[str, Any]:
    ...
    if result is not None and result.quote is not None:
        hash_now = preview_content_hash(
            result.quote, cart_id=cart.id, policy_version=registry.policy_version()
        )
    return {
...
def _assert_unchanged(
    ...
    Recompute-and-compare, under the cart's own row lock, so what is compared is what
    ...
    raise ProblemError(
        ...
        "The cart, the price or the catalogue moved after this was prepared, so it was "
...
def cart_body(cart: Cart, *, registry: MerchantRegistry) -> dict[str, Any]:
    ...
    body = CartOut(
        ...
        stale=cart.catalogue_revision is not None and cart.catalogue_revision != store.revision,
    )
...
def load_cart(
    session: Session, ctx: RequestContext, cart_id: uuid.UUID, *, lock: bool = False
) -> Cart:
    """The buyer's own cart, or 404.

    404 rather than 403 for a cart belonging to somebody else, for the same reason
    :func:`commerce_api.deps.assert_owner` does it: a 403 confirms the identifier exists
    and turns this endpoint into an oracle for enumerating other buyers' carts.
    ...
    """
    statement = select(Cart).where(Cart.id == cart_id, Cart.tenant_id == ctx.tenant_id)
    if lock:
        statement = statement.with_for_update()
    cart = session.execute(statement).scalar_one_or_none()
    if cart is None or cart.buyer_ref != ctx.buyer_ref:
        raise ProblemError(
            404,
            "Cart not found",
            "No cart with that identifier belongs to this session.",
            cart_id=str(cart_id),
        )
    return cart


def lock_cart(session: Session, ctx: RequestContext, cart_id: uuid.UUID) -> Cart:
    """The buyer's cart, held ``FOR UPDATE`` for the rest of this transaction."""
    return load_cart(session, ctx, cart_id, lock=True)
...
def create_cart(
    ...
    """Open an empty cart for this session's buyer.
    ...
    let a cart be priced against a merchant the buyer never chose.
    """
    cart = Cart(
        id=uuid7(),
...
def set_line(
    ...
    *,
    ...
    sku: str,
    quantity: int,
    expected: ExpectedCart | None = None,
) -> dict[str, Any]:
    ...
    The cart row is locked before its lines are read, so two surfaces editing one
    cart serialize instead of one silently overwriting the other.

    ``expected`` is optional and is the buyer's own +/- button's absence made explicit: a
    tap on the cart page has no proposal behind it and nothing to be stale against, so
    ...
    cart = lock_cart(session, ctx, cart_id)
    if cart.status != "OPEN":
    ...
    if len(lines) > MAX_BASKET_LINES:
        raise ProblemError(
            422,
            "Cart too large",
            f"A cart may hold at most {MAX_BASKET_LINES} distinct products.",
            field="sku",
            lines=len(lines),
        )

    cart.lines = [{"sku": key, "quantity": value} for key, value in sorted(lines.items())]

    result = quote_lines(cart.lines, store)
    if result is not None and result.quote is not None:
        cart.quote = result.quote.to_checkout_content()
        cart.catalogue_revision = result.quote.freshness.catalogue_revision
    else:
        # An unpriceable or empty cart stores no quote. Keeping the previous one would
