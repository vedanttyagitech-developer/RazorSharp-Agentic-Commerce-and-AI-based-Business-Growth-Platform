...
class SessionProvenance:
    ...
    def from_state(cls, state: object) -> SessionProvenance:
        ...
        try:
            ...
            # unhandled error instead of yielding the empty record this promises. The cases
            # loop above is inside the same guard for the same reason: a poisoned blob must
            # yield an empty record, never an exception out of a tool call.
            return cls()
        return record
