...
def verify_reply(
    ...
    *,
    ...
    # Every one of these reads ``scanned`` rather than ``reply``. The fold is what stops a
    # homoglyph or a fullwidth digit walking a claim past the check -- and it has to cover
    # the scarcity and unit-count checks too, not only the ones it was written for, or the
    # cheapest bypass of an invented "only 2 left" is to spell the 2 differently.
    ungrounded_counts = tuple(
        units for units in extract_stock_counts(scanned) if not ledger.knows_stock_count(units)
    )
