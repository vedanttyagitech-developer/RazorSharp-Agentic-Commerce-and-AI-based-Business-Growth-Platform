...
import {useChatFollow} from '@/lib/use-chat-follow';
...
import {visibleAgentReply} from '@/lib/chat-presentation';
...
import { ArrowLeft,ArrowRight,ArrowUpRight,ShoppingCart,PackageCheck,Plus,Minus,X,Check } from 'lucide-react';
...
function exitProjectMode(){sessionStorage.removeItem('razorsharp:tour');sessionStorage.removeItem('razorsharp:tour-step');sessionStorage.removeItem('razorsharp:tour-resume');sessionStorage.removeItem('razorsharp:tour-explicit');window.dispatchEvent(new Event('razorsharp:tour-stop'))}
...
function ShopWorkspace(){
 ...
 useEffect(()=>{if(sessionStorage.getItem('razorsharp:tour')==='on'&&sessionStorage.getItem('razorsharp:tour-explicit')!=='yes')sessionStorage.removeItem('razorsharp:tour')},[]);
 ...
 const turns=messages.slice(0,-1);const displayedReply=visibleAgentReply(voice.reply??messages.at(-1)?.reply);
...
// Dead code that fabricates an order identity is one refactor away from being live again.
 ...
 setStage(reviewOpen?'approval':'basket');const event={id:++changeSequence.current,name:item.name,delta:result.delta,quantity:result.quantity};setBasketUpdates([event]);if(result.delta>0&&!reviewOpen)setCartOpen(true);
 ...
 const send=(text:string)=>{if(navigateShop(text))return;if(requestsCheckoutReview(text)){voice.interrupt();setReviewRequested(true);return}responseTarget.current=chat.append(text);setView('discover');
  ...
  voice.say(text)};
 // Consume a homepage request once, after the existing cart is ready.
 const consumeHomeRequest=useEffectEvent(()=>{const url=new URL(window.location.href);const ask=url.searchParams.get('ask');const startVoice=url.searchParams.get('startVoice')==='1';if(!ask&&!startVoice)return;url.searchParams.delete('ask');url.searchParams.delete('startVoice');window.history.replaceState(window.history.state,'',url.href);if(ask)send(ask);else voice.startListening()});
 useEffect(()=>{if(cartReady&&!cartBusy){const frame=requestAnimationFrame(()=>consumeHomeRequest());return()=>cancelAnimationFrame(frame)}},[cartReady,cartBusy]);
 ...
 const newChat=()=>{exitProjectMode();responseTarget.current=null;setBasketUpdates([]);voice.interrupt();response.reset();chat.open(null);setSearch('');setView('discover')};
 ...
 const {latestRef,resultsRef}=useChatFollow(`${chat.active?.id??''}:${messages.length}:${query}`,voice.reply===null?'':`${voice.reply}:${shown.map(p=>p.id).join(',')}`,view==='discover'&&!!query&&!reviewOpen);
 ...
 return <SidebarProvider className="shopping-shell" style={{'--sidebar-width':'248px'} as React.CSSProperties}><Sidebar className="chat-sidebar store-sidebar"><SidebarHeader><Brand/><span className="store-sidebar-label">ECOMMERCE STORE</span><button className="new-chat" onClick={newChat}><Plus size={17}/> New conversation <span>⌘ K</span></button></SidebarHeader><SidebarContent><SidebarGroup><SidebarMenu>{([['discover','Start conversation'],['orders','Purchase history & orders'],['reserve','Reserve Pay']] as const).map(([id,t])=><SidebarMenuItem key={id}><SidebarMenuButton isActive={view===id} aria-pressed={view===id} data-destination={id} onClick={()=>{if(id==='discover')exitProjectMode();setView(id)}}>{id==='orders'&&<PackageCheck size={18} aria-hidden="true"/>}<span>{t}</span></SidebarMenuButton></SidebarMenuItem>)}</SidebarMenu></SidebarGroup><SidebarGroup><PreviousPayments onOrder={()=>setView('orders')} onConfirmed={()=>{}}/></SidebarGroup><SidebarGroup><SidebarMenu><SidebarMenuItem><SidebarMenuButton isActive={view==='support'} data-destination="support" onClick={()=>setView('support')}><span>Get help</span></SidebarMenuButton></SidebarMenuItem></SidebarMenu></SidebarGroup><SidebarGroup><SidebarGroupLabel>CONVERSATIONS</SidebarGroupLabel><small>Saved in this browser</small><div className="chat-history">{chat.history.chats.length===0?<p>Your saved conversations will appear here.</p>:[...chat.history.chats].reverse().map(c=><button key={c.id} aria-current={c.id===chat.history.activeId?'true':undefined} onClick={()=>{responseTarget.current=null;voice.interrupt();voice.reset();chat.open(c.id);setView('discover')}}>{c.title}</button>)}</div>{chat.notice&&<output>{chat.notice}</output>}</SidebarGroup></SidebarContent><SidebarFooter><Link href="/" className="workspace-switch"><span className="avatar store-profile-avatar">VT</span><div><strong>Vedant Tyagi</strong><span>Back to platform</span></div><ArrowUpRight size={15}/></Link></SidebarFooter></Sidebar><SidebarInset><div data-detail-open={product?.id} className={`buyer-app chat-first ${!query&&view==='discover'?'empty-chat':''}`}>
  <header className="app-header"><div className="header-left"><button type="button" className="workspace-icon-button header-back-button" onClick={goBack} aria-label="Go back" title="Back"><ArrowLeft size={17}/></button><SidebarTrigger/><span className="copilot-title">Ecommerce Store</span></div><div className="header-right"><MotionToggle/><button className="cart-toggle" onClick={()=>setCartOpen(!cartOpen)} aria-label={`Open cart, ${stage==='confirmed'?0:count} items`}><ShoppingCart size={19}/><span key={count}>{stage==='confirmed'?0:count}</span></button></div></header>
  {(!cartReady||cartBusy>0)&&<output>{cartReady?'Updating your cart…':'Restoring your cart…'}</output>}
  {cartReady&&(!!cartError||(paymentLocked&&!reviewOpen))&&<section className="cart-recovery-action" aria-label="Cart recovery"><span>{paymentLocked?'Existing checkout in progress.':'Your cart needs attention.'}</span><button className="secondary" disabled={cartBusy>0} onClick={()=>startNewCart(()=>setCartOpen(true))}>Start a new cart</button></section>}
  ...
  <div className={`buyer-layout ${cartOpen?'with-basket':''}`}>
   <main className={`buyer-main ${view==='discover'?'copilot-chat-area':''}`}>
   {view==='discover'&&<>
    <div className={`shop-intro ${query?'has-conversation':''}`}>
     {!query?<ShoppingShowcase onSelect={text=>transition(()=>send(text))}/>:<>{turns.map((t,i)=><div className="previous-turn" key={i}><div className="user-bubble">{t.user}</div>{visibleAgentReply(t.reply)&&<p className="assistant-message-bubble">{visibleAgentReply(t.reply)}</p>}</div>)}<div ref={latestRef} className="user-bubble latest-chat-turn">{query}</div>{displayedReply&&<div className="agent-answer" key={query}><div><span className="agent-name">Razor AI</span><AnswerText animate={false} text={displayedReply}/><span className="source-note"><Check size={13}/> {voice.reply!==null?'Assistant response · store catalogue':'Saved reply · ask again for current prices and availability'}</span></div></div>}<ResponseActivity phase={response.phase} live/></>}

    </div>
    {query&&<>{voice.supportOrder&&<section className="support-handoff panel"><h3>Review your support request</h3><p>Your assistant found the order. Review the case details before sending them to the merchant. No refund or cancellation has been executed.</p><button className="primary" onClick={()=>{setSupportOrder(voice.supportOrder!);setView('support')}}>Review order and contact merchant</button></section>}{(voice.reply!==null||voice.items.length>0)&&<section ref={resultsRef} className="recommendation-story compact-product-results" key="store-results" aria-label="Copilot product recommendations"><header hidden={!shown.length}><span><span className="store-dot"/> RazorSharp Store edit</span><span>{`${shown.length} finds`} · live catalogue</span></header><ul className="product-runway" hidden={!shown.length} aria-label="Recommended products">{shown.map((p,i)=><li className="runway-item" key={p.id} style={{'--card-order':i} as React.CSSProperties}><ProductCard detailOpen={product?.id===p.id} product={p} quantity={basket[p.id]||0} disabled={!cartReady||cartBusy>0||paymentLocked||stage==='checking'||stage==='confirmed'} onRemove={source=>add(p.id,-1,source)} onView={()=>setProduct(p)} onAdd={source=>add(p.id,1,source)}/></li>)}</ul>{!shown.length&&(catalogueLoading||catalogueError)&&<Empty title={catalogueLoading?'Fetching the shelf…':catalogueError?'The store is unreachable.':'Let’s try another thought.'} description={catalogueLoading?'Asking the store what it has today.':catalogueError||'Ask for milk, fruit, coffee or another everyday essential.'}/>}<div className="sr-only" role="log" aria-live="polite" aria-label="Cart updates" aria-relevant="additions">{basketUpdates.map(update=><div key={update.id} className={update.delta>0?'basket-chat-added':'basket-chat-removed'}><span>{update.delta>0?<Plus size={13}/>:<Minus size={13}/>}</span><p><strong>{update.delta>0?'Added':'Removed'} {Math.abs(update.delta)} · {update.name}</strong><span>{update.quantity?`${update.quantity} in your basket`:'Removed from your basket'}</span></p></div>)}</div>{shown.length>1&&<ProductComparison products={shown}/>}<div hidden={!shown.length} className="recommendation-caption"><span>{beyond>0?`Picked for this conversation · ${beyond} more on the shelf, ask for anything.`:'Picked for this conversation.'}</span></div>{count>0&&<div className="conversation-basket" style={{viewTransitionName:reviewOpen?undefined:'order-surface'}}><div className="basket-peek-images">{lines.slice(0,3).map(p=><ProductArt key={p.id} product={p}/>)}</div><div><strong>{count} {count===1?'good find':'good finds'}</strong><span>{money(total)} · delivery included</span></div><button onClick={()=>transition(reviewOrder)}>Review order <ArrowRight size={17}/></button></div>}</section>}</>}
    <div className={`conversation-composer store-chat-dock ${!query?'store-chat-home':''}`}><Composer keepExpanded assistantMessage={query?voice.reply:null} showSuggestions={!query} onSend={send} phase={query&&response.phase==='idle'?'complete':response.phase} onStop={response.stop}/></div>
   </>}
   ...
   {cartOpen&&<><button className="cart-backdrop" aria-label="Close cart" onClick={()=>setCartOpen(false)}/><aside className="basket-rail"><div className="basket-header"><div className="store-cart-heading"><div><h2>Your cart <span>{count}</span></h2></div></div><button className="icon-button" aria-label="Close cart" onClick={()=>setCartOpen(false)}><X size={19}/></button></div>
    {stage==='confirmed'?<div className="basket-success"><span className="success-ring"><Check size={33}/></span><Badge tone="green">Demo order confirmed</Badge><h2>Good choice.<br/>Great morning.</h2><p>Your order is confirmed. View the recorded payment and purchase details.</p><Primary onClick={()=>{setView('orders');setCartOpen(false)}}>View your order <ArrowRight size={16}/></Primary><button className="subtle" onClick={()=>setProof(true)}>View transaction proof</button><button className="subtle" onClick={()=>{startNewCart()}}>Start a new cart</button></div>:stage==='checking'?<div className="basket-success"><div className="checking-orbit"/><h2>Checking the payment.</h2><p>Waiting for provider evidence. A browser callback alone never confirms a purchase.</p><Badge tone="amber">Please don’t pay again</Badge></div>:<>{!count?<div className="store-cart-empty"><Empty title="Your cart is ready." description="Ask Razor AI to find something, then add the products you want. Review the exact bill before payment."/></div>:<><div className="basket-lines">{lines.map(p=><div className="basket-line" key={p.id}><div className="basket-product-art" style={{background:p.color}}><Image src={p.image} alt={p.name} unoptimized width={640} height={640}/></div><div className="basket-line-info"><strong>{p.name}</strong><span>{p.unit.split(' · ')[0]}</span><div className="quantity"><button aria-label={`Remove one ${p.name}`} disabled={!cartReady||cartBusy>0||paymentLocked} onClick={e=>add(p.id,-1,e.currentTarget.closest('.basket-line') as HTMLElement)}><Minus size={12}/></button><span>{basket[p.id]}</span><button aria-label={`Add one ${p.name}`} disabled={!cartReady||cartBusy>0||paymentLocked} onClick={e=>add(p.id,1,e.currentTarget.closest('.basket-line') as HTMLElement)}><Plus size={12}/></button></div></div><strong>{money(p.price*basket[p.id])}</strong></div>)}</div><div className="basket-totals"><div><span>Item subtotal</span><span>{money(subtotal)}</span></div><div><span>Delivery</span><span>{delivery?money(delivery):'On us'}</span></div>{discount>0&&<div className="green-text"><span>{quote?.offer_label||'Merchant offer'}</span><span>−{money(discount)}</span></div>}<div><span>Tax</span><span>{quote?money(quote.items_tax_minor+quote.delivery_tax_minor):'—'}</span></div><div className="total-line"><strong>Total</strong><strong key={total} className="amount-update">{money(total)}</strong></div><p>Prices and totals from the merchant’s current quote.</p></div>
    <Primary className="wide" onClick={()=>transition(reviewOrder)}>Review order <ArrowRight size={17}/></Primary></> }</>}
   </aside></>}
  </div>
