...
function Tilt({ children, className = '' }: {children: React.ReactNode; className?: string}) {
  return <a href={className.includes('merchant') ? '/merchant' : '/shop'} className={`portal ${className}`} onPointerMove={e => { const b=e.currentTarget.getBoundingClientRect(); e.currentTarget.style.setProperty('--rx', `${-(e.clientY-b.top-b.height/2)/65}deg`); e.currentTarget.style.setProperty('--ry', `${(e.clientX-b.left-b.width/2)/65}deg`); }} onPointerLeave={e=>{e.currentTarget.style.setProperty('--rx','0deg');e.currentTarget.style.setProperty('--ry','0deg');}}>{children}</a>;
}
export default function Home() {
 return <div className="platform-page has-identity">
  <header className="platform-header"><a className="wordmark branded-wordmark" href="/">razorsharp<span className="platform-brand-label">platform</span></a><div className="platform-motion"><MotionToggle/><span className="prototype-pill"><i/> Meet Razor AI</span></div><nav className="hl-header-nav" aria-label="Platform navigation"><a href="#why-razorsharp">20 features</a><a href="#track-one">Track 1</a><a href="#highlights">Highlights</a><a href="#trust-boundaries">Trust & boundaries</a><a className="hl-nav-open" href="#copilots">Explore copilots <ArrowUpRight size={15}/></a></nav></header>
  <IdentityHero/><ImpactDeck/><main className="platform-main" id="copilots">
   ...
   <div className="portal-grid">
    <Tilt className="buyer-portal"><img src="/quick-commerce-hero.png" alt="Quick-commerce assortment: electronics, personal care, home essentials, stationery, toys, snacks and groceries" className="portal-photo"/><div className="portal-content"><span className="portal-kicker"><ShoppingBag size={16}/> FOR YOUR EVERYDAY</span><h2>A little less searching.<br/>A lot more finding.</h2><p>Everyday tech, beauty, home and groceries. One Razor AI shopping copilot.</p><span className="portal-cta">Meet your shopping copilot <ArrowUpRight size={21}/></span></div><span className="floating-note"><Sparkles size={15}/> “Earphones, skincare, and snacks for tonight.”</span></Tilt>
