...
export function PossibilityScene({open,onOpenChange}:{open:boolean;onOpenChange:(open:boolean)=>void}){
 ...
 <article className="space-window window-shop"><header><i/> YOUR EVERYDAY <span>01</span></header><p>“Breakfast for two,<br/>under ₹500.”</p><div className="space-products">{['AMUL-DAIRY-001','BANA-PROD-006','BRIT-BAKE-001'].map(p=><Image key={p} src={`/products/${p}.webp`} alt={p} onError={e=>{e.currentTarget.style.display='none'}} unoptimized width={640} height={640}/>)}</div><Link href="/ecommerce-store">Find your everyday <ArrowUpRight size={16}/></Link></article>
 ...
 <article className="space-window window-growth"><header><i/> MERCHANT COMMAND <span>02</span></header><p>Make your next<br/><em>big move.</em></p><div className="space-bars">{[30,48,39,65,54,78,71,95].map((v,i)=><i key={i} style={{height:v+'%',animationDelay:i*.1+'s'}}/>)}</div><Link href="/merchant">Explore your business <ArrowUpRight size={16}/></Link></article>
