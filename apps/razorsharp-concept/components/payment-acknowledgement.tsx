...
export function PaymentAcknowledgement({orderId,compact=false}:{orderId:string;compact?:boolean}){
 ...
 return <section aria-label="Payment acknowledgement" className={`payment-acknowledgement${compact?' reserve-payment-receipt':''}`}>
  <header><ShieldCheck size={22}/><div><small>PAYMENT RECORD</small><h3>{compact?'Your payment receipt':'Payment acknowledgement'}</h3></div></header>
  ...
  {data&&<><span className="ack-mode">{data.mode==='test'?'TEST MODE · NO REAL MONEY':data.mode==='simulation'?'SIMULATED · NO REAL MONEY':data.mode==='live'?'LIVE PAYMENT':'MODE NOT VERIFIED'}</span>
   ...
   <button className="primary" onClick={download}><Download size={16}/> {compact?'Download your receipt':'Download acknowledgement'}</button>
