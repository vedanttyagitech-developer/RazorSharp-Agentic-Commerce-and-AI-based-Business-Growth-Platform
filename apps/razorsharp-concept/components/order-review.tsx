...
type ReviewProps={onRecoveredOrder:(order:import('@/lib/commerce').Order)=>void;onFreshReview:(id:string)=>Promise<void>;cartId?:string;cartBusy:boolean;onContinue:(text:string)=>void;open:boolean;onClose:()=>void;lines:Product[];basket:Record<string,number>;subtotal:number;discount:number;delivery:number;total:number;offer:boolean;stage:string;onQuantity:(id:string,delta:number,source?:HTMLElement)=>void;onManualConfirmed:(result:ManualConfirmation)=>void;onReserveConfirmed:(result:ReserveConfirmation)=>void;onOffer:()=>void;onOrder:()=>void;onProof:()=>void;onPaymentLock:(locked:boolean)=>void};

export function OrderReview({onRecoveredOrder,onFreshReview,cartId,cartBusy,onContinue,open,onClose,lines,basket,subtotal,discount,delivery,total,stage,onQuantity,onManualConfirmed,onReserveConfirmed,onOrder,onProof,onPaymentLock}:ReviewProps){
 ...
 const lastCheckout=useRef<string|undefined>(undefined);if(authoritative)lastCheckout.current=authoritative.checkout_id;
 ...
 useEffect(()=>{if(stage==='basket')setHandoff('choose')},[stage]);
 ...
 return <Dialog open={open} onOpenChange={v=>{if(v)return;onClose()}}><DialogContent className="checkout-shell"><div style={{viewTransitionName:'order-surface'}} className={`order-review-dialog checkout-card ${confirmed?'is-confirmed':''}`}>
  ...
  {bill.state.status==='recovering'&&!confirmed?<CheckoutRecovery onConfirmed={order=>{setPlaced({orderId:order.order_id,reference:order.reference});onRecoveredOrder(order)}} onFreshReview={onFreshReview} initial={bill.state.view} onOrder={onOrder}/>:!confirming&&!confirmed&&handoff==='reserve'?<ReserveCheckout onFreshReview={onFreshReview} reviewed={authoritative as unknown as ReviewCard|null} commitRequest={reserveCommit} onReviewState={setReserveBill} total={showTotal} lines={lines} basket={basket} onGuidance={setReserveGuidance} onConfirmed={r=>{setPlaced({orderId:r.orderId,reference:null,remainingMinor:r.remainingMinor});onReserveConfirmed(r)}} onBack={()=>{onPaymentLock(false);setHandoff('choose')}}/>:!confirming&&!confirmed&&handoff==='manual'&&authoritative?<ManualCheckout onFreshReview={onFreshReview} card={authoritative} onGuidance={setManualGuidance} onConfirmed={r=>{setPlaced({orderId:r.orderId,reference:r.reference});onManualConfirmed(r)}} onBack={()=>{onPaymentLock(false);setHandoff('choose')}}/>:confirming||confirmed?<div className="review-outcome">
   ...
   {placedOrder.status==='loading'&&<p className="review-price-note" role="status">Reading how long this transaction took…</p>}
   ...
   {bill.state.status==='building'&&<p className="review-price-note" role="status">Building your order with the store — {bill.state.step==='cart'?'opening a cart':bill.state.step==='lines'?'adding your items':'pricing and holding stock'}.</p>}
   ...
   {priced&&<p className="review-price-note">Order {authoritative.checkout_id.slice(0,8)} · version {authoritative.version}{authoritative.reservation?` · stock held until ${new Date(authoritative.reservation.expires_at).toLocaleTimeString()}`:''}</p>}
 ...
 </div>{open&&<CheckoutAssistant version={authoritative?.version??reserveBill?.version} checkoutId={authoritative?.checkout_id??(bill.state.status==='recovering'?bill.state.view.checkout_id:lastCheckout.current)} onNext={onContinue} onManual={chooseManual} onReserve={chooseReserve} checkoutStage={confirmed?'success':bill.state.status==='recovering'?'verifying':confirming?'verifying':handoff==='manual'?'manual':handoff==='reserve'?(reserveBill?'reserve-review':'verifying'):'review'} total={reserveBill?.amount_minor??showTotal} subtotal={reserveBill?.quote.items_subtotal_minor??showSubtotal} tax={(reserveBill?.quote.items_tax_minor??q?.items_tax_minor??0)+(reserveBill?.quote.delivery_tax_minor??q?.delivery_tax_minor??0)} delivery={reserveBill?.quote.delivery_fee_minor??showDelivery} discount={reserveBill?.quote.discount_minor??showDiscount} message={confirmed?'Your payment is confirmed and your order is placed. What would you like to do next?':bill.state.status==='recovering'?'Checking your existing checkout. Please do not pay again until its status is verified.':handoff==='manual'&&!confirming&&!confirmed?manualGuidance:handoff==='reserve'&&!confirming&&!confirmed?reserveGuidance:confirmed?'Your test payment is confirmed, and your demo order has been placed. What would you like to do next?':confirming?'Checking the simulated payment and preparing your demo order. Please do not pay again.':`How would you like to pay ${money(showTotal)}: manually through Razorpay checkout, or with AI-assisted Reserve Pay? Tell me your choice, or use a button. Reserve Pay uses a simulated provider here.`}/>}
