...
import {flyToBasket,transition} from './continuity';
...
export function Brand(){return <a href="/" className="wordmark branded-wordmark">razorsharp<span className="platform-brand-label">platform</span></a>}
...
export function Composer({onSend,placeholder='Or type what’s on your mind…',compact=false,phase='idle',onStop,visualMode}:{visualMode?:WaveMode;onSend:(v:string)=>void;placeholder?:string;compact?:boolean;phase?:ResponsePhase;onStop?:()=>void}){
 ...
 useEffect(()=>{if(voice.phase==='ready')setValue(voice.transcript)},[voice.phase,voice.transcript]);
 const submit=()=>{if(!busy&&value.trim()){voice.interrupt();transition(()=>onSend(value.trim()));setValue('');setEditing(false)}};
 const voiceActive=voice.phase==='listening'||voice.phase==='speaking'||voice.phase==='transcribing';
 ...
 const label=voice.phase==='listening'?'Listening':voice.phase==='speaking'?'Speaking':voice.phase==='transcribing'?'Transcribing':voice.phase==='ready'?'Ready to send':busy?(compact&&phase==='searching'?'Reading your business':phase==='thinking'?'Thinking':phase==='preparing'?'Preparing your answer':responseLabels[phase]):'Your voice. Your pace.';
 ...
 return <form className={`composer voice-first-composer ${compact?'compact':''} ${busy?'is-working':''} ${voiceActive?'voice-active':''}`} data-phase={mode} data-engaged={editing||voiceActive||busy} data-collapsed={collapsed} onSubmit={e=>{e.preventDefault();submit()}}>
   <div className="composer-corner-lights" aria-hidden="true"><i/><i/><i/><i/></div><div className="voice-composer-top"><span className="voice-state-label" role="status"><i/>{label}</span><span className="voice-demo-label">{voice.live?'VOICE CONNECTED':'RAZOR AI'}</span></div>
...
export function ProductCard({product,onAdd,onRemove,onView,detailOpen=false,quantity=0,disabled=false}:{detailOpen?:boolean;product:Product;onAdd:()=>boolean|void;onRemove?:(source:HTMLElement)=>void;onView:()=>void;quantity?:number;disabled?:boolean}){
 const tilt=useSurfaceTilt();const [added,setAdded]=useState(false);const timer=useRef<ReturnType<typeof setTimeout>|null>(null);
 useEffect(()=>()=>{if(timer.current)clearTimeout(timer.current)},[]);
 useEffect(()=>{const node=tilt.ref.current;if(!node)return;node.dataset.reveal='pending';const observer=new IntersectionObserver(([entry])=>{if(entry.isIntersecting){node.dataset.reveal='visible';observer.disconnect()}},{threshold:.12});observer.observe(node);return()=>observer.disconnect()},[tilt.ref]);
