...
export function requestsCheckoutReview(text:string):boolean{
 ...
 return /^(please\s+)?(checkout|check out|go to checkout|take me to checkout|take me to review screen|proceed to checkout|proceed (to|for) payment|proceed to pay|go to payment|take me to payment|continue to payment|review (my |the )?(order|bill|cart)|show (me )?(my |the )?(bill|checkout)|place (my |the )?order)(\s+please)?$/.test(s)
  || /^(checkout|payment|पेमेंट|भुगतान|चेकआउट)\s+(pe|par|पे|पर)\s+(chal|chalo|chale|chalein|chalen|chaliye|chalte hain|le chalo|चल|चलो|चलें|चलिए)$|^(review|रिव्यू)( (karo|kar do|करो))?$/.test(s)
