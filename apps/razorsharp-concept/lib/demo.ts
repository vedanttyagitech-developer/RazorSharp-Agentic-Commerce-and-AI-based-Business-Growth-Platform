...
export type Product = {id:string;sku:string;name:string;unit:string;price:number;category:string;symbol:string;color:string;stock:number;isListed?:boolean;isAvailable?:boolean;image:string};
...
export const capabilities=[['Shopping & search','Catalogue APIs','Available backend'],['Cart & quantity','Cart APIs','Available backend'],['Checkout & consent','Atomic approval + admission','Available backend'],['Payment & reconciliation','Provider evidence + worker','Available backend'],['Merchant actions','Six supported action kinds','Available backend'],['Merchant Policy','Versioned publication','Available backend'],['Customer support','Case intake + merchant queue','Available backend'],['Transaction proof','Proof + audit verification APIs','Available backend'],['Voice conversation','Browser voice, STT and spoken responses; requires configured speech services','Connected'],['Merchant copilot','Grounded reads and draft proposals; owner approval required','Connected'],['Item refund plans','Durable Resolution Plan confirmation','Planned experience'],['Reserve Pay','Saved product permissions, Kernel admission and durable execution','Server-side provider simulation'],['Sales analytics','Recorded confirmed order value; attribution unavailable','Connected'],['Gmail','Gmail API integration planned','Future / disabled']];
