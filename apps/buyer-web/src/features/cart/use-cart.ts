/**
 * The cart, as the rest of the storefront sees it.
 *
 * Three surfaces write to one cart -- a product card, the cart page, and RazorAI
 ...
 * quantity, and the response is the whole re-quoted cart, which is stored verbatim.
 ...
 * that overtake each other would otherwise leave the cart holding whichever response
...
import type { Cart, Unavailability } from "@/lib/api/types";

/** Mirrors `cart_service.MAX_LINE_QUANTITY`, so the refusal is legible before the round trip. */
const MAX_LINE_QUANTITY = 99;

/**
 * Where the remembered product names live, and why they are allowed to live there.
 *
 * `providers.tsx` sets the rule this follows: the cart identifier and the line count are
 ...
 * stale one costs a buyer nothing. It is kept only so that a buyer who reloads a cart
 ...
 *
 * The cart identifier is restored from `localStorage`, which the server could not read,
 * so during hydration this hook is looking at a snapshot that says "no cart" whether or
 * not the buyer has one. Rendering "your cart is empty" from that snapshot would be a
 * claim about the buyer's cart that nothing has checked. Once React has hydrated it
 ...
 * off the same `cart` the caller is rendering, so `available_units` is no less the
 ...
 */
export function unavailableBySku(cart: Cart | null): Map<string, Unavailability> {
  if (!cart) return new Map();
  return new Map(cart.unavailable.map((entry) => [entry.sku, entry]));
}

/**
 * True when the cart this browser remembers no longer exists on the server.
 *
 * Two shapes mean that: a 404 naming a cart (the API restarted, or the row was never
 * this buyer's), and a 409 naming a status (the cart was consumed into a checkout).
 * A 404 that names a SKU is a different fact entirely -- the product does not exist --
 * and must not be recovered from by silently opening a new cart.
 */
function isBasketGone(error: unknown): boolean {
  if (!(error instanceof ApiError)) return false;
  if (error.status === 404) return error.problem.cart_id !== undefined;
  if (error.status === 409) return error.problem.basket_status !== undefined;
  return false;
}

export function useCart(): UseCart {
  const { cartId, itemCount, setCartId, setLineCount, refresh } = useCartContext();
  const hydrating = useHydrating();

  const [stored, setStored] = useState<Cart | null>(null);
  /**
   * The last name each SKU was quoted under, kept across responses.
   *
   * A cart line is `{sku, quantity}`; the human name of a product reaches this screen
   * only on a quote row. So when the merchant declines to price the cart, the quote is
   ...
   * cart has come back for it yet, and deriving that rather than storing it keeps this
  ...
  /*
   * A cart answers for exactly one identifier. Matching the two here, rather than
   * clearing the state whenever the identifier changes, keeps the lines of a cart that
   * has just been left behind from being shown in the frame between a new identifier
   * arriving and its first quote coming back.
   */
  const cart = stored !== null && stored.cart_id === cartId ? stored : null;

  // The chained writes below run outside the render that scheduled them, so they read
  // the current cart through refs rather than through a captured closure.
  const basketIdRef = useRef<string | null>(cartId);
  const chainRef = useRef<Promise<void>>(Promise.resolve());
  /**
   * One idempotency key per (cart, sku, quantity) attempt, held until that attempt
  ...
  const quantities = useMemo<Record<string, number>>(() => {
    ...
    for (const line of cart?.lines ?? []) merged[line.sku] = line.quantity;
  ...
  /**
   * Store a cart the server sent, and learn the product names it came with.
   *
   * Every write of the cart goes through here so no response can update the lines
   * without also updating the names, which is how the two would drift apart.
   */
  const acceptCart = useCallback((next: Cart): void => {
    setStored(next);
    setLearnedNames((current) => {
      const held = new Set(next.lines.map((line) => line.sku));
      const merged: Record<string, string> = {};
      // Names for SKUs still in the cart, so the map cannot grow without bound and a
  ...
  const basketRef = useRef<Cart | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    if (cart) setLineCount(cart.lines.length);
    else if (!cartId) setLineCount(0);
  }, [cart, cartId, setLineCount]);

  useEffect(() => {
    if (!cartId) return;
    const controller = new AbortController();
    api
      .cart(cartId, controller.signal)
      .then((next) => {
      ...
      .catch((cause: unknown) => {
        if (controller.signal.aborted) return;
        // A cart the server has forgotten is not an error the buyer can act on. Drop
        // the identifier; the next add opens a fresh one.
        if (isBasketGone(cause)) setCartId(null);
  ...
  const openBasket = useCallback(async (): Promise<string> => {
    ...
    setCartId(created.cart_id);
    acceptCart(created);
    return created.cart_id;
  }, [acceptCart, setCartId]);

  /** One line write, with a single recovery attempt when the cart has gone away. */
  const writeLine = useCallback(
    async (id: string, sku: string, quantity: number): Promise<Cart> => {
      let target = id;
      // Two passes at most, written as a bounded loop rather than a recursive call so the
      // bound is visible: the second pass exists only for a cart the server no longer
      ...
      for (let pass = 0; pass < 2; pass += 1) {
        ...
        try {
          ...
          // That key belongs to a cart that no longer exists. The retry is a different
          // request against a different cart, so it earns a key of its own; a key kept
  ...
  // Another surface can write this cart and ask the provider to re-read it: the press on
  ...
  // A cart identifier with nothing read against it yet is still loading, whichever
  ...
  const settling = cartId !== null && cart === null && error === null;
