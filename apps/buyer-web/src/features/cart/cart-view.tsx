/**
 * The cart screen: what the buyer has, what it costs, and what happens next.
 *
 ...
 * `stale` -- the store moved while this cart was open -- is shown as a notice, because a
 ...
 *
 * "Proceed to checkout" opens a checkout and hands off. From that moment the cart is
 * closed server-side: it has become checkout version 1, with its own reservation and its
 * own frozen policy receipt, so the stored cart identifier is dropped here rather than
...
import { unavailableBySku, useCart } from "./use-cart";
...
export function CartView() {
  ...
  const { cartId, cart, names, loading, error, busySku, setQuantity, reload } = useCart();
  ...
  const proceed = useCallback(async () => {
    ...
    try {
      const checkout = await api.openCheckout(cartId, handoffKey.current);
      // The server has closed this cart into version 1. Forget it here too, so a later
      // add opens a new cart instead of writing to one that can only answer 409;
      ...
      // kept `/cart`'s policy and Razorpay's script was refused on arrival. See
  ...
  if (loading && !cart) return <LoadingBasket />;
   ...
   *
   * `cart.unavailable` is the merchant saying so; the quote's silence is not. This
   * screen used to mark every line missing from `quote.lines` as unavailable, which is
   * correct only while a quote exists. `merchant_sim.fees.quote_basket` refuses a cart
  ...
  const refused = unavailableBySku(cart);
  const declined = cart.unavailable;
    ...
    <div className="space-y-4">
      ...
      {cart.stale ? (
        <Notice tone="amber" title="This cart was re-priced">
          The merchant&rsquo;s catalogue moved while the cart was open, so every amount below has been
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-start">
        ...
        >
          {/*
            One pass over the cart's own lines, in the cart's own order. Each row is
        ...
        <div className="space-y-3 lg:sticky lg:top-4">
          {quote ? (
            ...
            <Notice tone="amber" title="This cart has no total yet">
              ...
              <span className="mt-1 block font-mono text-[11px] text-[var(--ink-4)]">code {cart.code}</span>
            </Notice>
          )}
