/**
 * The basket screen: the line the merchant took away, and the line it merely capped.
 *
 ...
 * screen that *quietly* stops being about the basket the buyer has. Four ways that can
 ...
 *     refuses a basket *whole*: `merchant_sim.fees.quote_basket` returns no quote at all
 ...
 *     three it had visibly declined. `basket.unavailable` is the merchant naming the line
 ...
 *  4. **A failure passes for a state.** A read that failed and a basket that is empty look
 *     identical if the screen is careless, and so do "the merchant re-priced this" and
 *     "these were always the figures".
 *
 * Every body below was captured from the live API on 2026-09-05 by driving a real basket.
 ...
 *
 * The harness mocks `@/lib/api/client`, not `./use-basket`, so the real hook runs: the
 ...
 * different basket into it, and the tests would run in an order that mattered. What it
...
/** The second capture's basket. A basket answers for one identifier, so it needs its own. */
const CAPPED_BASKET_ID = "01a0715c-ffa1-7297-a31f-eed873277d94";

/**
 * Where `use-basket` keeps the last name each SKU was quoted under.
 *
 ...
 * is true and `available_units` is 20 — so this is a basket one tap from being priced,
 * and every figure the buyer needs to make that tap is in `unavailable`.
 */
const BUTTER_CAPPED: Basket = {
  basket_id: CAPPED_BASKET_ID,
  lines: [
...
/**
 * The names those three SKUs were last quoted under, as `use-basket` would have stored
 * them from the responses that priced this basket before the twenty-first butter.
 *
 * A basket line is `{sku, quantity}`; the human name of a product reaches this screen only
 * on a quote row, and there is no quote in the body above. Seeding storage is how a buyer
 * arrives at a refused basket in real life — they had it priced a moment ago, or they
 ...
 * Every basket row carries the product's photograph — the muted ones included, greyed —
...
describe("a line the merchant delisted while the basket was open", () => {
  ...
  it("keeps the delisted line on screen instead of dropping it between renders", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Remove India Gate Classic Basmati Rice 5 kg from the basket" }),
    ).toBeDefined();
  ...
  it("says the merchant declined the rice, and does not say it declined the milk", async () => {
     ...
     * basket rather than about the product, and the bill panel states it there.
     */
...
describe("one line over the merchant's stock", () => {
   ...
   * the merchant is still selling it and the basket is one tap from being priced. This is
   ...
   */
  beforeEach(() => {
    mocks.context.basketId = CAPPED_BASKET_ID;
    window.localStorage.setItem(NAMES_KEY, JSON.stringify(REMEMBERED_NAMES));
    mocks.api.basket.mockResolvedValue(BUTTER_CAPPED);
  });
  ...
  it("leaves the other two lines with their names and their steppers", async () => {
    ...
    for (const name of ["Aashirvaad Shudh Chakki Atta 5 kg", "Act II Butter Popcorn 150 g"]) {
      ...
      // account of the basket than the one the buyer had a moment ago.
