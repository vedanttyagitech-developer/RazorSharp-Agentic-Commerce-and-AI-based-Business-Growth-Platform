    ...
    <article className="space-y-6">
      ...
      <nav aria-label="Breadcrumb" className="text-xs font-semibold text-stone-500">
        <ol className="flex items-center gap-1.5 flex-wrap">
          ...
          <li>
            <Link href={`/?category=${product.category}`} className="hover:text-stone-900 transition capitalize">
              {product.category === "electronics" ? "Mobile Phones" : product.category}
            </Link>
          </li>
          <li aria-hidden="true" className="text-stone-400">›</li>
          <li className="font-bold text-stone-900 line-clamp-1 max-w-sm sm:max-w-md" aria-current="page">
            {product.name_en}
          </li>
        </ol>
      </nav>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start">
        ...
        <div className="lg:col-span-6 flex gap-3 sm:gap-4 items-start">
          ...
          <div className="flex flex-col gap-2 shrink-0">
            {allImages.length > 0 ? (
              allImages.slice(0, 6).map((imgUrl, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`h-14 w-14 sm:h-16 sm:w-16 rounded-2xl border flex items-center justify-center p-1.5 transition-all bg-white cursor-pointer ${
                      isActive
              ...
              <div className="h-14 w-14 rounded-2xl border border-stone-200 flex items-center justify-center text-2xl">
                {categoryEmoji}
              </div>
            )}
            ...
            {allImages.length > 1 && (
              <button
                ...
                className="h-6 w-full flex items-center justify-center text-xs font-bold text-stone-400 hover:text-stone-900 transition cursor-pointer"
          ...
          <div className="flex-1 space-y-4">
            <div className="relative aspect-square rounded-3xl border border-stone-200 bg-white flex items-center justify-center p-8 sm:p-12 shadow-xs overflow-hidden">
              <SafeImage
              ...
              <button
                ...
                className="absolute right-3.5 bottom-3.5 flex h-8 w-8 items-center justify-center rounded-full border border-stone-200 bg-white text-stone-600 shadow-xs hover:bg-stone-50 transition cursor-pointer"
                aria-label="Next angle"
              >
                ›
              </button>
            ...
            <div>
              {isAvailable ? (
                ...
                <button
                  type="button"
                  disabled
                  className="w-full h-12 sm:h-14 rounded-2xl bg-stone-200 text-stone-500 font-bold text-base cursor-not-allowed"
                >
        ...
        <div className="lg:col-span-6 rounded-3xl border border-stone-200/80 bg-white p-6 sm:p-7 space-y-5 shadow-2xs">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-stone-500 hover:text-stone-900 cursor-pointer">
                {brand} ›
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-stone-200 bg-[#fbfbfb] p-3 flex items-center gap-3 shadow-2xs">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-stone-100 text-stone-700 text-lg">
                🛡️
              </div>
              <div>
          ...
          <div className="rounded-2xl border border-stone-200 bg-white p-4 space-y-2.5 shadow-2xs">
          ...
          <div className="rounded-2xl border border-stone-200 bg-stone-50/60 p-4 space-y-2 text-xs">
            <h3 className="font-bold text-stone-900">Governed Product Facts</h3>
            <p className="text-stone-700 font-semibold">
              Applicable GST: {formatBasisPoints(product.tax_bp)}
            </p>
            <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-stone-600">
              <dt className="text-stone-500">SKU:</dt>
              <dd className="font-mono font-medium text-stone-900">{product.sku}</dd>

              <dt className="text-stone-500">Category:</dt>
              <dd className="capitalize text-stone-900">{product.category}</dd>

              <dt className="text-stone-500">Stock Units:</dt>
              <dd className="tabular-nums text-stone-900">{product.stock_units}</dd>
            </dl>
