 ...
 * checkout on the trusted surface — the same `POST /v1/baskets/{id}/checkout` the basket
 ...
 * to:
 *
 * 1. **The button exists only when the proposal names a basket** (and a handler is passed).
 *    A checkout with no basket to build from is not a proposal anyone can act on, so the
 *    card draws only the door.
 * 2. **A press calls the client method once, with that basket id** — nothing recomputed,
 *    nothing invented.
...
/**
 * `POST /v1/baskets/{id}/checkout` answers **201 with version 1's approval card**, not a
...
/** The `checkout.create` envelope the shopping specialist proposes once it has read a basket. */
...
/** The same proposal with no basket in context — nothing a press could open. */
...
describe("the checkout card's press exists only when there is a basket to open", () => {
  ...
  it("draws no button when the proposal names no basket, handler or not", () => {
    render(<CheckoutProposalCard basketId={null} onConfirm={vi.fn()} navigate={vi.fn()} />);
    expect(screen.queryAllByRole("button")).toHaveLength(0);
  });
  ...
  it("gives the proposal card a real button only when the panel wired a handler with a basket", () => {
    ...
    // there is no basket to open and none to link to.
    rerender(
      <ProposalCard
...
describe("a press opens the named checkout, once, and lands the buyer on its approval page", () => {
  it("calls the client method exactly once with the proposal's basket id, then navigates to the returned checkout", async () => {
    ...
    // Navigated to the checkout the server opened, by its own id — never guessed from the basket.
